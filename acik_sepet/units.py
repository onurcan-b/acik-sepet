...
def parse_quantity(title: str, expected: str) -> Quantity | None:
    ...
    if expected in {"mass", "volume"}:
        pattern = r"(\d+(?:\.\d+)?)\s*[x×]\s*(\d+(?:\.\d+)?)\s*(kg|kilogram|g|gr|gram|ml|cl|mililitre|mililiter|l|lt|litre|liter)\b"
        ...
        pattern = r"(\d+(?:\.\d+)?)\s*(kg|kilogram|g|gr|gram|ml|cl|mililitre|mililiter|l|lt|litre|liter)\b"
    ...
    if expected == "count":
        count_unit = r"(?:adet|ad\.?|li|lu|rulo|rol|demet|bag|paket)"
        multipack = re.search(rf"(\d+)\s*[x×]\s*(\d+)\s*{count_unit}\b", text)
        if multipack:
        ...
        patterns = [
            r"(\d+)\s*(?:adet|ad\.?)\b",
            r"(\d+)\s*['’]?\s*(?:li|lu)\b",
            r"(\d+)\s*(?:rulo|rol|demet|bag|paket)\b",
        ]
        for pattern in patterns:
            match = re.search(pattern, text)
            if match:
                return Quantity(float(match.group(1)), "count")
        if re.search(r"\b(?:adet|rulo|rol|demet|bag|paket)\b", text):
