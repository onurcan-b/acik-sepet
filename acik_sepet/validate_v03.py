...
def validate(min_type_coverage: float = 0.60, min_skus: int = 300) -> None:
    ...
    viable = [type_id for type_id, count in by_type.items() if type_id in spec_by_id and count >= spec_by_id[type_id]["min_skus"]]
    type_coverage = len(viable) / max(1, len(specs))
    duplicates = [key for key, type_ids in key_types.items() if len(type_ids) > 1]

    print(
        f"snapshot={latest.name} rows={len(rows)} observed_types={len(by_type)}/{len(specs)} "
        f"viable_types={len(viable)}/{len(specs)} viable_coverage={type_coverage:.1%} duplicates={len(duplicates)} bad_prices={bad_prices}"
    )
    ...
    if duplicates:
        raise SystemExit(f"Aynı SKU birden fazla ürün tipinde: {duplicates[:5]}")
    if bad_prices:
        raise SystemExit(f"Geçersiz birim fiyat sayısı: {bad_prices}")
